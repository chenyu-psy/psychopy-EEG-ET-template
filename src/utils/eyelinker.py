...
import pylink as pl
try:
    # Package import (preferred when `src` is a package root)
    from .eyelink_display import EyeLinkDisplay
except ImportError:
    # Script-style fallback used by legacy entry points
    from utils.eyelink_display import EyeLinkDisplay
try:
    from .color_utils import to_rgb255
except ImportError:
    from utils.color_utils import to_rgb255
...
def _display_not_connected_text(window):
    ...
    Parameters:
    window -- a visual.Window
    """
    warning_text = (
        "WARNING: Eyetracker not connected.\n\n"
        'Press "R" to retry connecting\n'
        'Press "Q" to quit\n'
        'Press "D" to continue in debug mode'
    )

    bg = visual.Rect(window, units="norm", width=2, height=2, fillColor="#000000", colorSpace="hex")
    text_stim = visual.TextStim(window, warning_text, color="#FFFFFF", colorSpace="hex")
...
def _get_connection_failure_response():
    """Returns a key press."""
    keys = event.waitKeys(keyList=["r", "q", "d"])
    if not keys:
        # Defensive fallback for static type checkers; waitKeys should block until a key is pressed.
        return "q"
    return keys[0]
...
def EyeLinker(window, filename, eye, text_color=None):
    ...
    Parameters:
    window -- A visual.Window object
    filename -- EDF filename, max 12 characters with extension
    eye -- Which eye(s) to track, either "LEFT", "RIGHT" or "BOTH"
    text_color -- HEX color string (e.g., "#FFFFFF"). If None, auto-selects black/white.
    """
    connected, e = _try_connection()

    if connected:
        return ConnectedEyeLinker(window, filename, eye, text_color=text_color)
    else:
    ...
    if response == "q":
        ...
        return MockEyeLinker(window, filename, eye, text_color=text_color)
...
class ConnectedEyeLinker:
    ...
    def __init__(self, window, filename, eye, text_color=None):
        ...
        self.genv = EyeLinkDisplay(self.window, self.tracker)
        self.mock = False
    ...
    def initialize_graphics(self):
        """Opens the EyeLinkDisplay object.
    ...
    def send_tracking_settings(self, settings=None):
        ...
        pl.setCalibrationColors(to_rgb255(settings["foreground_color"]), to_rgb255(settings["background_color"]))
        pl.setCalibrationSounds(settings["target_sound"], settings["good_sound"], settings["error_sound"])

        if self.eye in ("LEFT", "RIGHT"):
            self.send_command("active_eye = %s" % self.eye)

        self.send_command("automatic_calibration_pacing = %i" % settings["automatic_calibration_pacing"])
        ...
        self.send_command("enable_automatic_calibration = %s" % settings["enable_automatic_calibration"])
        ...
        self.send_command("saccade_acceleration_threshold = %i" % settings["saccade_acceleration_threshold"])
        self.send_command("saccade_motion_threshold = %f" % settings["saccade_motion_threshold"])
    ...
    def display_eyetracking_instructions(self):
        ...
        visual.Circle(self.window, units="pix", radius=18, lineColor="#000000", fillColor="#FFFFFF", colorSpace="hex").draw()
        visual.Circle(self.window, units="pix", radius=6, lineColor="#000000", fillColor="#000000", colorSpace="hex").draw()
    ...
    def calibrate(self, text=None):
        ...
        visual.TextStim(
            self.window, text=text, pos=(0, 0), height=0.05, units="norm", color=self.text_color, colorSpace="hex"
        ).draw()
    ...
    def gaze_data_both(self):
        ...
        Always contains a nested tuple with gaze data.
        Each eye's tuple of gaze data contains an x and y value in pixels, or is None if the eye is not recorded.
        Can be accessed with `tracker.gaze_data_both`
...
_method_list = [
    fn_name
    for fn_name in dir(ConnectedEyeLinker)
